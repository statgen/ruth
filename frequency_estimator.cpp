...
frequency_estimator::frequency_estimator(Eigen::MatrixXd *_pEigVecs, double _tol, double _maxLambda) : skipIf(false), skipInfo(false), siteOnly(false), nelderMead(false), assumeHWD(false), gtError(0.005)
{
  if (_pEigVecs == NULL)
...
frequency_estimator::frequency_estimator(Eigen::BDCSVD<Eigen::MatrixXd> *_pSVD, double _tol, double _maxLambda) : skipIf(false), skipInfo(false), siteOnly(false), gtError(0.005)
{
  // set dimensions;
  pEigVecs = NULL;
...
bool frequency_estimator::set_hdr(bcf_hdr_t *_hdr, bcf_hdr_t *_wdr)
{
  if (hdr != _hdr)
  {
    ...
    if (!skipInfo)
    {
      ...
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=HWE_SLP_P,Number=1,Type=Float,Description=\"Signed log p-value of HWE test with pooled allele frequencies\">\n");
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "FIBC_P") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=FIBC_P,Number=1,Type=Float,Description=\"Inbreeding coefficient with pooled allele frequencies\">\n");
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "HWE_SLP_I") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=HWE_SLP_I,Number=1,Type=Float,Description=\"Signed log p-value of HWE test with individual-sepcific allele frequencies\">\n");
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "FIBC_I") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=FIBC_I,Number=1,Type=Float,Description=\"Inbreeding coefficient with individual-sepcific allele frequencies\">\n");
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "MAX_IF") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=MAX_IF,Number=1,Type=Float,Description=\"Maximum Individual-specific allele frequency\">\n");
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "MIN_IF") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=MIN_IF,Number=1,Type=Float,Description=\"Minimum Individual-specific allele frequency\">\n");
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "BETA_IF") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=BETA_IF,Number=%d,Type=Float,Description=\"Coefficients for intercept and each eigenvector to obtain ISAF\">\n", ndims);
        bcf_hdr_append(wdr, buffer);
      }
      if (bcf_hdr_id2int(_hdr, BCF_DT_ID, "LLK0") < 0)
      {
        snprintf(buffer, sizeof(buffer), "##INFO=<ID=LLK0,Number=1,Type=Float,Description=\"Null likelihood - for debug purpose\">\n");
        bcf_hdr_append(wdr, buffer);
      }
    }
    if ((!skipIf) && (!siteOnly))
    {
      snprintf(buffer, sizeof(buffer), "##FORMAT=<ID=IF,Number=1,Type=Float,Description=\"Individual-specific allele frequencies\">\n");
      bcf_hdr_append(wdr, buffer);
    }
...
{ //, std::vector<int32_t>* p_icols) {
  ...
  else
  {
    ...
    else if (field.compare("GT") == 0)
    {
      ...
      // gtError is per-allele error rate
      // use a simple model for gtError
      // when geno=0, use 0, e, 2e
      // when geno=1, use e, 0, e
      // when geno=2, use 2e, e, 0
      int32_t phredErr = (int32_t)floor(-10 * log10(gtError) + 0.5);
      int32_t errM[9] = {0, phredErr, phredErr * 2, phredErr, 0, phredErr, phredErr * 2, phredErr, 0};
      ...
      else
      {
        ...
        if (pls == NULL)
          pls = new int32_t[nsamples * 3];
        for (int32_t i = 0; i < nsamples; ++i)
        {
          int32_t geno = bcf_gt_allele(gts[2 * i]) + bcf_gt_allele(gts[2 * i + 1]);
          if (bcf_gt_is_missing(gts[2 * i + 1]))
          { // haploid or missing
            if (bcf_gt_is_missing(gts[2 * i]))
            { // missing
              pls[3 * i] = pls[3 * i + 1] = pls[3 * i + 2] = 0;
              continue;
            }
            else
            { // pretend to be homozygous for haploid
...
double frequency_estimator::estimate_pooled_af_em(int32_t maxiter)
{
  if (pooled_af < 0)
  {
    ...
    pooled_af = p;
    if (pooled_af * nsamples < 0.5)
    {
      pooled_af = 0.5 / (1 + 2 * nsamples);
    }
    else if ((1 - pooled_af) * nsamples < 0.5)
    {
      pooled_af = 1 - 0.5 / (1 + 2 * nsamples);
    }
  }
  return pooled_af;
}
...
double frequency_estimator::Evaluate(Vector &v)
{
  ...
  for (int32_t i = 0; i < nsamples; ++i)
  {
    ...
    isaf = expGeno / 2.0;
    if (isaf * (nsamples + nsamples + 1) < 0.5)
      isaf = 0.5 / (nsamples + nsamples + 1);
    if ((1.0 - isaf) * (nsamples + nsamples + 1) < 0.5)
      isaf = 1.0 - 0.5 / (nsamples + nsamples + 1);
    isafQ = 1.0 - isaf;
...
void frequency_estimator::estimate_isaf_em(int32_t maxiter)
{
  ...
  if (!isaf_computed)
  {
    ...
    { // maxiter = 30
      ...
      for (int32_t j = 0; j < nsamples; ++j)
      {
        if (isaf[j] * (nsamples + nsamples + 1) < 0.5)
          isaf[j] = 0.5 / (nsamples + nsamples + 1.0);
        else if ((1.0 - isaf[j]) * (nsamples + nsamples + 1) < 0.5)
          isaf[j] = 1.0 - 0.5 / (nsamples + nsamples + 1.0);
      }
    }
  ...
  if (!isaf_computed)
  {
    ...
    { // maxiter = 30
      ...
      for (int32_t j = 0; j < nsamples; ++j)
      {
        ...
        {                         // this might be buggy for multi-allelics
      ...
      for (int32_t j = 0; j < nsamples; ++j)
      {
        if (isafD[j] < minAF)
          isafD[j] = minAF;
        else if (1.0 - isafD[j] < minAF)
          isafD[j] = 1.0 - minAF;
        if (isafE[j] < minAF)
          isafE[j] = minAF;
        else if (1.0 - isafE[j] < minAF)
          isafE[j] = 1.0 - minAF;
      }
